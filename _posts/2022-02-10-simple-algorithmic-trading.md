...
### Goal
My goal here is to build a very simple trading algorithm that can trade crypto (although it can be used to trade any asset) and show how to: 1.anlyse the trading result, 2. make changes accordingly and 3. eventually deploy to run on the cloud and show real time results.

Most people think that a trading algorithm should be somehing very sophisticated with thousands of lines of code and some complicated machine learning that can only be done usgin GPU. In reality, some are using complicated AI for trading, and some AI are making money while some are not. 
I will save my thoughts on why some suceed and some do not for another blog post, but the point is that you can also write a trading algorithm bot that will run on a very simaple code (simplae logic) AND it will still work. 
